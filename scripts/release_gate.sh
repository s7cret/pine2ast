...
run_log() {
  local log="$1"
  shift
  echo "[release-gate] start $log"
  if "$@" >"$OUT_DIR/$log" 2>&1; then
    echo "[release-gate] ok $log"
  else
    local status=$?
    echo "[release-gate] failed $log" >&2
    tail -80 "$OUT_DIR/$log" >&2 || true
    return "$status"
  fi
}
...
rm -f .coverage
run_log TEST_RUN_FINAL.log env PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 "$PYTHON" -m pytest -p pytest_cov tests/unit tests/integration --cov=pine2ast --cov-report=term-missing --cov-report=xml:"$OUT_DIR/coverage.xml"
...
run_log SIGNATURE_COVERAGE_V5_FINAL.log "$PYTHON" -m pine2ast.semantic.signature_coverage --version 5 --fail-on-missing --fail-under-signature-ready-ratio 1.0 --json "$OUT_DIR/SIGNATURE_COVERAGE_V5_FINAL.json"
run_log SIGNATURE_COVERAGE_V6_FINAL.log "$PYTHON" -m pine2ast.semantic.signature_coverage --version 6 --fail-on-missing --fail-under-signature-ready-ratio 1.0 --json "$OUT_DIR/SIGNATURE_COVERAGE_V6_FINAL.json"
run_log ARCHITECTURE_BUDGET_FINAL.log "$PYTHON" -m pine2ast.quality architecture pine2ast --max-lines 700
run_log RELEASE_FEATURES_FINAL.log "$PYTHON" -m pine2ast.compatibility.release_features
run_log CONTRACT_VALIDATION_FINAL.log "$PYTHON" -m pine2ast.contracts.validation tests/fixtures/valid/declarations/basic_indicator.pine --json "$OUT_DIR/CONTRACT_VALIDATION_FINAL.json"
run_log DISTRIBUTION_MANIFEST_FINAL.log "$PYTHON" -m pine2ast.distribution manifest --root . --json "$OUT_DIR/DISTRIBUTION_MANIFEST_FINAL.json"
run_log RELEASE_MANIFEST_FINAL.log "$PYTHON" -m pine2ast.release --root . --json "$OUT_DIR/RELEASE_MANIFEST_FINAL.json"
