...
def _mismatch(label: str, expected: object, actual: object) -> LanguagePublicationError:
    return LanguagePublicationError(f"{label} mismatch: published {expected!r}, installed {actual!r}")


PRIMARY_STAGE2_CRITERIA = (
    "versioned_catalog", "stateful_language_matrix", "imports", "independent_builtin_expected",
)
MANDATORY_STAGE2_GATES = (
    "python_3.11", "python_3.13", "sandbox", "protected_workers", "stage1_corpus",
    "architecture", "frontend", "package_builds", "test_inventory",
)

def _verify_full_acceptance(published: Mapping[str, Any], evidence_root: Path | None = None) -> None:
    ...
    if any(not isinstance(row, Mapping) or row.get("status") != "resolved" or not row.get("evidence_hash")
    ...
    if any(criteria.get(key) != "accepted" for key in PRIMARY_STAGE2_CRITERIA):
        raise LanguagePublicationError("full acceptance requires all four accepted primary criteria")
    ...
    for name in MANDATORY_STAGE2_GATES:
        ...
        if (not isinstance(receipt, Mapping) or receipt.get("status") != "passed"
            ...
            raise LanguagePublicationError(f"full acceptance lacks clean exact-tree receipt: {name}")
...
def verify_language_publication(
    ...
    *,
    ...
    if published.get("packages", {}).get("pine2ast") != packages.get("pine2ast"):
        raise _mismatch("pine2ast version", published.get("packages", {}).get("pine2ast"), packages.get("pine2ast"))
    missing = []
    ...
    if published.get("catalog_hashes") != observed.get("catalog_hashes"):
        raise _mismatch("catalog hashes", published.get("catalog_hashes"), observed.get("catalog_hashes"))
    for field in ("compiler_target_hash", "compiler_target_name", "compiler_target_version", "runtime_target_file_hash"):
    ...
    if mode == "coordinated" and missing:
        raise LanguagePublicationError("coordinated publication lacks observations: " + ", ".join(missing))
    if published.get("full_stage2_accepted") is True:
        _verify_full_acceptance(published, evidence_root)
        if missing:
            raise LanguagePublicationError("full acceptance requires complete coordinated observations")
    return {"status": "published" if mode == "coordinated" else "verified_local",
            "publication": published["publication"], "mode": mode,
            "observation_complete": not missing, "missing_observations": missing,
