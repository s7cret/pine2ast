"""F2.3: mixed-import `and`/`or` keep origin numeric-condition policy.

Catalog: v5 ``numeric_condition_allowed=true``, v6 false.
``if 1`` is tests/stage2/test_stage2_origin_numeric_condition.py.
This file covers ``1 and 0`` inside an exported library body: operand
admission must follow the library ``//@version``, not the consumer.
"""
...
def test_v6_consumer_v5_library_numeric_and_is_admitted() -> None:
    linked = _linked(6, 5)
...
def test_v6_same_version_library_numeric_and_is_rejected() -> None:
    linked = _linked(6, 6)
    parsed = parse_code(linked.code, ParseOptions(library_context=linked.qualifier_context()))
    assert not parsed.ok
    assert codes.TYPE_MISMATCH in {d.code for d in parsed.diagnostics}
    try:
        build_consumer_bundle(linked.code, linked_source=linked)
    except ConsumerBundleError:
        return
    raise AssertionError("v6 library numeric and must fail consumer-bundle admission")


def test_v5_same_version_library_numeric_and_is_admitted() -> None:
    linked = _linked(5, 5)
    parsed = parse_code(linked.code, ParseOptions(library_context=linked.qualifier_context()))
    assert parsed.ok, [(d.code, d.message) for d in parsed.diagnostics]
    assert build_consumer_bundle(linked.code, linked_source=linked)["content_hash"]
