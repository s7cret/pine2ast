...
def test_local_explicit_receiver_has_full_types_and_exact_parameter_indices(
    version, call, qualifier
):
    ...
    assert (
        next(a for a in fact["arguments"] if a["parameter_name"] == "self")["parameter_index"] == 0
    )
...
def test_exported_result_floor_survives_explicit_namespace_call(v):
    with pytest.raises((LibraryError, owner.ConsumerBundleError)):
        checked(
            "n=m.fixed(1)\nx=input.int(n)\nplot(x)",
            {"u/M/1": lib("export method fixed(simple int self)=>7", v=v)},
            v=v,
        )
...
def test_resealed_proofs_still_require_fresh_ast_semantics(monkeypatch, qualifier, fault):
    ...
    if fault == "receiver":
        ...
        call["arguments"][0]["actual_qualifier"] = "simple"
    elif fault == "default":
    ...
    def forbidden(*args, **kwargs):
        raise AssertionError(
            "source-free verification must replay AST, not parse unavailable source"
        )
...
def test_capability_set_must_match_actual_explicit_call_syntax(fault):
    bundle = owner.build_consumer_bundle(
        '//@version=6\nindicator("caps")\nmethod plus(int self)=>self+1\nplot(plus(2))\n'
    )
    contract = bundle["consumer_contract"]
    if fault == "missing":
        contract["required_capabilities"].remove(CAP)
    elif fault == "duplicate":
        contract["required_capabilities"].append(CAP)
    elif fault == "minimum":
        contract["minimum_consumer_version"] = "4.0.0"
    else:
        contract["required_capabilities"].append("unknown_method_feature")
    reseal(bundle)
    with pytest.raises(owner.ConsumerBundleError):
        owner.verify_consumer_bundle(
            bundle, expected_producer_commit=bundle["producer"].get("commit")
        )
