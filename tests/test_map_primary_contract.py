...
def test_put_previous_value_has_the_map_value_type(version, form, named, dtype, value):
    ...
    expression = (
        f"map.put(id=a,{args})"
        if form == "function" and named
        else f"map.put(a,{args})" if form == "function" else f"a.put({args})"
    )
    ...
    call = next(
        ...
        if c["symbol_id"] == f"pine:{form}:map.put"
    )
    assert call["return_type"] == dtype
    assert {a["parameter_name"] for a in call["arguments"]} == (
        {"id", "key", "value"} if form == "function" else {"key", "value"}
    )
...
@pytest.mark.parametrize(
    "expression,form",
    [
        ("map.put_all(a,b)", "function"),
        ("map.put_all(id=a,id2=b)", "function"),
        ("map.put_all(id2=b,id=a)", "function"),
        ("a.put_all(b)", "method"),
        ("a.put_all(id2=b)", "method"),
    ],
)
def test_put_all_primary_source_names(version, expression, form):
    ...
    call = next(
        c
        for c in semantic_facts_payload(parsed)["calls"]
        if c["symbol_id"] == f"pine:{form}:map.put_all"
    )
    assert call["return_type"] == "void"
    assert {a["parameter_name"] for a in call["arguments"]} == (
        {"id", "id2"} if form == "function" else {"id2"}
    )
    assert build_consumer_bundle(source)["content_hash"]
...
@pytest.mark.parametrize(
    "expression",
    [
        "map.put_all(id=a,other=b)",
        "map.put_all(id=a,from=b)",
        "a.put_all(other=b)",
        "a.put_all(from=b)",
        "map.put_all(id=a,id2=3)",
        "map.put_all(id=a,id2=b,id2=b)",
        "map.put(id=a,key=3,value=2)",
        'map.put(id=a,key="a",value="bad")',
    ],
)
