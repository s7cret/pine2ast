...
assert (
    ...
    == "bb7c32877244350a0fc1f92d7172eba55626352610cd6b965826d0a62297c1aa"
)
...
def test_primary_parameter_admission_and_exact_binding(case, binding):
    ...
    if binding == "named":
        ...
        after = (
            ...
            + case["function"]
            + "("
            + ",".join(name + "=" + value for name, value in zip(names, args))
            + ")"
        )
    ...
    if not case["primary_source_admission_expected"]:
        ...
        assert any(
            d.code == "P2A1405" and ("Argument " + case["parameter"] + " ") in d.message
    ...
    call = next(
        ...
        if row["callee"] == "ta." + case["function"]
    )
    ...
    assert call["return_type"] == (
        "tuple<float,float,float>" if case["function"] == "macd" else "float"
    )
