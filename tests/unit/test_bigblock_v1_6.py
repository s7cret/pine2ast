...
''')
    assert codes.SYNTAX_ERROR not in errors
    switch_nodes = [n for n in res.ast.items if isinstance(n, VarDeclaration) and isinstance(n.initializer, SwitchStructure)]
    assert switch_nodes
...
''')
    ...
    assert res.ast is not None
    y_decl = next(item for item in res.ast.items if isinstance(item, VarDeclaration) and item.name == "y")
