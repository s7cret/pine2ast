...
from pine2ast.ast.nodes import EnumDeclaration, FunctionDeclaration, ImportDeclaration, MethodDeclaration, TupleDeclaration, TypeDeclaration


def parse_items(src):
    res = parse_code(src, ParseOptions(run_semantic=False))
    assert res.ast is not None, [d.to_dict() for d in res.diagnostics]
    assert not [d for d in res.diagnostics if d.severity.value in {"FATAL", "ERROR"}], [d.to_dict() for d in res.diagnostics]
