...
def test_v31_official_gap_is_closed_without_claiming_deferred_surfaces():
    ...
    assert report["namespaces"]["request"]["known_deferred"] == []
    assert "request.economic" in report["namespaces"]["request"]["known_unsupported"]
    assert "runtime.error" in report["namespaces"]["runtime"]["known_unsupported"]


def test_v31_request_economic_has_explicit_unsupported_contract():
    src = """//@version=6
indicator("econ")
