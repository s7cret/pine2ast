...
def test_v212_registry_backlog_has_parameter_metadata_for_priority_entries():
    ...
    required = [
        "line.new", "label.new", "box.new", "table.new", "polyline.new",
        "strategy.entry", "strategy.exit", "strategy.order", "request.security",
        "input.int", "input.float", "input.bool", "input.string", "input.timeframe",
    ]
    missing = [name for name in required if not registry["functions"].get(name, {}).get("parameters")]
    assert missing == []
...
def test_v212_compile_oracle_metadata_is_schema_v2_internal_checked():
    payload = json.loads(Path("tests/fixtures/compile_oracle/strategy_namespace/metadata.json").read_text(encoding="utf-8"))
    assert payload["schema_version"] == 2
    assert all(item["pine2ast_status"] == "pass" for item in payload["policy"])
    assert all(item["tradingview_status"] == "pending_external_oracle" for item in payload["policy"])
