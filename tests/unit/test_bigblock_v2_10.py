...
''', ParseOptions(strict_builtin_namespaces=True))
    assert not any(d.code == codes.UNKNOWN_BUILTIN_MEMBER and d.severity is Severity.ERROR for d in result.diagnostics)
...
plot(z)
''')
    narrowed = [scope.non_na_symbols for scope in result.semantic_model.scopes if "x" in scope.non_na_symbols]
    assert narrowed == [{"x"}]
