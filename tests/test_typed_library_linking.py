...
def linked(body, usage, version=6):
    result = link_libraries(
        root(usage, version), LibraryStore.create({"qa/Lib/1": library(body, version)})
    )
    result.verify()
    return result
...
def test_udt_constructor_fields_arguments_returns_and_explicit_annotation(version):
    ...
    assert (
        result.original_location(result.code.rindex(declarations["Point"]))["source"] == "<memory>"
    )
...
def test_same_name_types_from_distinct_publishers_and_revisions_stay_distinct(version):
    ...
    source = (
        source.replace("a=a.Point", "pa=a.Point")
        .replace("b=b.Point", "pb=b.Point")
        .replace("c=c.Point", "pc=c.Point")
        .replace("plot(a.x+b.x+c.x)", "plot(pa.x+pb.x+pc.x)")
    )
...
def test_transitive_type_fields_and_parameters_close_over_exact_type_identity():
    sources = {
        ...
        "qa/Lib/1": library(
            "import qa/Types/1 as t\nexport type Box\n    t.Point point\n    t.Direction direction=t.Direction.up\nexport read(t.Point p)=>p.x"
        ),
    }
    result = link_libraries(
        root(
            "p=t.Point.new(2)\nbox=lib.Box.new(p,t.Direction.down)\nplot(lib.read(box.point))",
            imports="import qa/Lib/1 as lib\nimport qa/Types/1 as t",
        ),
        LibraryStore.create(sources),
    )
    assert parse_code(result.code).ok
    assert set(result.dependency_hashes) == set(sources)
    assert result == link_libraries(
        root(
            "p=t.Point.new(2)\nbox=lib.Box.new(p,t.Direction.down)\nplot(lib.read(box.point))",
            imports="import qa/Lib/1 as lib\nimport qa/Types/1 as t",
        ),
        LibraryStore.create(dict(reversed(list(sources.items())))),
    )
    result.verify()
...
def test_reference_profile_is_monotonic_and_dependency_edits_invalidate_identity():
    ...
    assert (
        before.receipt()["profile"]
        == after.receipt()["profile"]
